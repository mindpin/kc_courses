...
feature "课件页面" do
  background do
    @course = KcCourses::Course.create(:title => '测试课程', :desc => '课程描述')
    @chapter = @course.chapters.create(:title => '测试章节', :desc => '章节描述')
  end
  ...
  scenario "列表跳转进入编辑" do
    other_ware = @chapter.wares.create(:title => '其他标题', :desc => '其他描述')
    visit "/chapters/#{@chapter.id}"
    click_link '编辑'
