...
feature "课件页面" do
  background do
    ...
    @course = @user.courses.create(:title => '测试课程', :desc => '课程描述')
    @chapter = @user.chapters.create(:title => '测试章节', :desc => '章节描述', course: @course)
  end
  ...
  describe 'after sign in' do
    ...
    scenario "列表跳转进入编辑" do
      other_ware = @user.wares.create(:title => '其他标题', :desc => '其他描述', chapter: @chapter)
      visit "/chapters/#{@chapter.id}"
      click_link '编辑'
    ...
    scenario "列表删除" do
      other_ware = @user.wares.create(:title => '其他标题', :desc => '其他描述', chapter: @chapter)
      visit "/chapters/#{@chapter.id}"
      expect(page).to have_content(other_ware.title)
      # 默认确认
      #accept_confirm do
      click_link '删除'
      #end
      expect(page).not_to have_content(other_ware.title)
      expect(current_path).to eq("/chapters/#{@chapter.id}")
    end
  end
end
