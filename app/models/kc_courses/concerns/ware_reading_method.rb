  ...
  module Concerns
    module WareReadingMethod
      ...
      def read_percent_of_user(user)
        return ware_readings.where(:creator_id => user.id.to_s).first.read_percent
      end

      # user 某一天内学习了 course/chapter/ware 多少百分比的内容，返回值是代表百分比的数字
      def read_percent_change_of_user(user, time)
        return ware_reading_deltas.where(:creator_id => user.id.to_s, :time => time).first.read_percent_change
      end
      
      # user 截止到某天（包括这个某天内学习的），已经学习了 course/chapter/ware 多少百分比的内容，返回值是代表百分比的数字
      def read_percent_of_user_before_time(user, time)
        return ware_reading_deltas.where(:creator_id => user.id.to_s, :time => time).first.read_percent
      end
    end
  end
end
